...
// ==========================================
// MOCK DATA SEMENTARA (Jangan Lupa Nanti Diganti dengan Data Soal Aslimu)
// ==========================================
const MOCK_PACKAGES = [
...
const getQuestionsForPackage = (pkgId) => {
  return [
    { id: 'q1', type: 'multiple_choice', text_html: 'Apa bahasa Jepangnya "Terima kasih"?', options: [{id:'o1', label:'A', text_html:'Arigatou', is_correct:true}, {id:'o2', label:'B', text_html:'Sayounara', is_correct:false}], explanation: 'Arigatou digunakan untuk menyatakan terima kasih.' }
  ];
};
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (currentUser.role === 'admin') {
      ...
      const unsubAttempts = onSnapshot(q, (snapshot) => {
        ...
        getDocs(userQ).then(res => { if(!res.empty) setCurrentUser(res.docs[0].data()); });
      });
  ...
  const handleAdminLogin = (e) => {
    ...
    } else { setLoginError("Ups! Username atau password admin salah."); }
  };

  const handleLogout = () => { setCurrentUser(null); setCurrentView('login'); setStudentName(''); setStudentClass(''); };

  const startExam = (pkgId) => {
    setCurrentAttempt({ id: `att_${Date.now()}_${Math.floor(Math.random()*1000)}`, userId: currentUser.id, pkgId: pkgId, status: 'in_progress', answers: {}, startedAt: Date.now(), score: 0, isTimeout: false });
    setCurrentView('exam');
  };

  const abortExam = () => { setCurrentAttempt(null); setCurrentView('student_dash'); };

  const submitExam = async (forcedTimeout) => {
    const currentQuestions = getQuestionsForPackage(currentAttempt.pkgId);
    let correct = 0;
    currentQuestions.forEach(q => {
      if (q.type === 'freewriting') { if (checkFreewritingAnswer(currentAttempt.answers[q.id], q.accepted_answers)) correct++; } 
      else { const correctOpt = q.options.find(o => o.is_correct); if(correctOpt && currentAttempt.answers[q.id] === correctOpt.id) correct++; }
    });

    const score = (correct / currentQuestions.length) * 100;
    const passed = score >= 60;
    const finalAttempt = { ...currentAttempt, status: 'submitted', score, passed, submittedAt: Date.now(), correctCount: correct, isTimeout: forcedTimeout };
    ...
    } catch (err) { console.error("Gagal menyimpan ke Cloud:", err); }
  };
  ...
  return (
    <div className="min-h-screen font-sans text-[#4B4B4B] bg-[#F7F7F7] flex justify-center">
      ...
      <div className={`w-full bg-white min-h-screen relative flex flex-col shadow-2xl ${currentUser?.role === 'admin' ? 'max-w-6xl' : 'max-w-md'}`}>
        ...
        {currentView === 'login' && (
          <LoginView loginTab={loginTab} setLoginTab={setLoginTab} studentName={studentName} setStudentName={setStudentName} studentClass={studentClass} setStudentClass={setStudentClass} handleStudentLogin={handleStudentLogin} adminUser={adminUser} setAdminUser={setAdminUser} adminPass={adminPass} setAdminPass={setAdminPass} handleAdminLogin={handleAdminLogin} loginError={loginError} isLoading={isLoading} />
        )}
...
// ==========================================
// SUB-COMPONENTS FULL UI DUOLINGO
// ==========================================
...
function ExamView({ attempt, packages, questions, onSaveAnswer, onSubmit, onAbort }) {
  ...
  if (!currentQ) return <div className="p-10 text-center font-bold">Data soal kosong!</div>;

  const isFirst = currentIndex === 0;
  const isLast = currentIndex === questions.length - 1;
  const hasAnswer = attempt.answers[currentQ.id] !== undefined && attempt.answers[currentQ.id] !== '';
    ...
    <div className="flex-1 flex flex-col bg-white h-screen overflow-hidden relative">
      ...
      <div className="absolute bottom-0 left-0 right-0 border-t-2 border-[#E5E5E5] bg-white p-5 pb-8 flex justify-between items-center z-30">
        <div className="max-w-lg w-full mx-auto flex gap-4">
          <button onClick={() => setCurrentIndex(prev => prev - 1)} disabled={isFirst} className={`btn-3d py-4 px-6 rounded-2xl font-extrabold uppercase transition-all border-b-4 flex-shrink-0 ${isFirst ? 'bg-[#E5E5E5] border-[#CECECE] text-[#AFAFAF] cursor-not-allowed opacity-50' : 'bg-white border-[#E5E5E5] border-2 text-[#AFAFAF] hover:bg-[#F7F7F7]'}`}>MUNDUR</button>
          {isLast ? (
            ...
            <button onClick={() => setCurrentIndex(prev => prev + 1)} className={`btn-3d flex-1 py-4 rounded-2xl font-extrabold uppercase transition-all border-b-4 ${hasAnswer ? 'bg-[#58CC02] border-[#58A700] text-white hover:bg-[#4CEB00]' : 'bg-[#E5E5E5] border-[#CECECE] text-[#AFAFAF]'}`}>LANJUT</button>
          )}
        </div>
      </div>
    </div>
  ...
  return (
    <div className="flex-1 flex flex-col bg-white h-screen overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto px-5 py-8 custom-scrollbar">
        <h3 className="font-black text-2xl text-[#4B4B4B] mb-6 text-center">Cek Jawabanmu</h3>
        <div className="space-y-6 max-w-lg mx-auto">
          {questions.map((q, i) => {
            ...
            const userAnsOpt = q.options?.find(o => o.id === attempt.answers[q.id]);
            const corrAnsOpt = q.options?.find(o => o.is_correct);
            return (
              <div key={q.id} className="bg-white rounded-2xl border-2 border-[#E5E5E5] p-5 shadow-sm">
                ...
                <div className="bg-[#F7F7F7] rounded-xl p-4 mb-4 border-2 border-[#E5E5E5]">
                  <p className="text-[#AFAFAF] font-bold text-xs uppercase mb-1">Jawabanmu:</p>
                  <p className={`japanese-text font-bold ${isCorrect ? 'text-[#58CC02]' : 'text-[#FF4B4B]'}`} dangerouslySetInnerHTML={{ __html: userAnsOpt ? userAnsOpt.text_html : '<em>Kosong</em>' }} />
                  {!isCorrect && (<div className="mt-3 pt-3 border-t-2 border-[#E5E5E5]"><p className="text-[#AFAFAF] font-bold text-xs uppercase mb-1">Seharusnya:</p><p className="japanese-text font-bold text-[#58CC02]" dangerouslySetInnerHTML={{ __html: corrAnsOpt ? corrAnsOpt.text_html : '' }} /></div>)}
                </div>
              </div>
            );
          })}
...
function AdminDashboard({ users, attempts, packages, getQuestions, onLogout }) {
  ...
  const avgScore = subs.length ? subs.reduce((a, b) => a + b.score, 0) / subs.length : 0;
  
  const handleExport = () => {
    const headers = ["Nama Siswa", "Kelas", "Paket", "Status", "Nilai"];
    const rows = subs.map(att => `"${users.find(u => u.id === att.userId)?.name}","${users.find(u => u.id === att.userId)?.className}","${packages.find(p => p.id === att.pkgId)?.title}","${att.passed ? 'Lulus' : 'Remedial'}","${Math.round(att.score)}"`);
    const link = document.createElement("a"); link.setAttribute("href", encodeURI("data:text/csv;charset=utf-8," + [headers.join(","), ...rows].join("\n"))); link.setAttribute("download", "Rekap_Nilai.csv"); document.body.appendChild(link); link.click(); document.body.removeChild(link);
  };
  ...
  return (
    <div className="flex-1 flex flex-col bg-[#F7F7F7] min-h-screen">
      <div className="bg-white border-b-2 border-[#E5E5E5] px-8 py-5 flex justify-between items-center sticky top-0 z-50 shadow-sm">
        <div className="flex items-center"><div className="w-12 h-12 bg-[#FF4B4B] rounded-2xl border-b-4 border-[#EA2B2B] flex items-center justify-center mr-4"><span className="text-white font-black text-xl">虎</span></div><div><h1 className="font-black text-2xl text-[#4B4B4B] leading-none">Ruang Sensei</h1><p className="text-[#AFAFAF] font-bold text-sm mt-1">Pantau petarungmu di sini.</p></div></div>
      ...
      <div className="p-8 flex-1 grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-1 space-y-6">
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-white rounded-3xl p-6 border-2 border-[#E5E5E5] flex flex-col items-center text-center shadow-sm relative overflow-hidden"><span className="text-[#AFAFAF] font-extrabold uppercase text-xs tracking-wider mb-2 z-10">Total Murid</span><span className="text-5xl font-black text-[#1CB0F6] z-10">{students.length}</span><Users className="absolute -bottom-4 -right-4 w-24 h-24 text-[#1CB0F6]/5 z-0" /></div>
            <div className="bg-white rounded-3xl p-6 border-2 border-[#E5E5E5] flex flex-col items-center text-center shadow-sm relative overflow-hidden"><span className="text-[#AFAFAF] font-extrabold uppercase text-xs tracking-wider mb-2 z-10">Rata-rata</span><span className="text-5xl font-black text-[#58CC02] z-10">{Math.round(avgScore)}</span><Award className="absolute -bottom-4 -right-4 w-24 h-24 text-[#58CC02]/5 z-0" /></div>
          </div>
        </div>
        <div className="lg:col-span-2">
          <div className="bg-white rounded-3xl border-2 border-[#E5E5E5] overflow-hidden h-full flex flex-col shadow-sm">
            <div className="px-6 py-4 bg-[#F7F7F7] border-b-2 border-[#E5E5E5] flex justify-between items-center"><h3 className="font-black text-[#4B4B4B] uppercase tracking-wide">Progres Siswa Live</h3><button onClick={handleExport} className="btn-3d bg-[#58CC02] text-white px-4 py-2 rounded-xl text-sm font-extrabold border-b-4 border-[#58A700] hover:bg-[#4CEB00] flex items-center"><Download className="w-4 h-4 mr-2" /> EXCEL</button></div>
            <div className="flex-1 overflow-auto custom-scrollbar p-2">
              <table className="w-full text-left text-sm border-collapse">
                <thead className="text-[#AFAFAF] sticky top-0 bg-white shadow-sm z-10"><tr><th className="px-6 py-4 font-black">NAMA & KELAS</th><th className="px-6 py-4 font-black text-center">PAKET</th><th className="px-6 py-4 font-black text-center">SKOR</th><th className="px-6 py-4 font-black text-center">STATUS</th></tr></thead>
                <tbody>
                  {subs.map(att => (
                    <tr key={att.id} className="border-b-2 border-[#F7F7F7] hover:bg-[#F7F7F7]/50 transition-colors">
                      <td className="px-6 py-4"><div className="font-black text-[#4B4B4B] text-base">{users.find(u => u.id === att.userId)?.name}</div><div className="text-[#AFAFAF] font-bold text-xs mt-0.5">{users.find(u => u.id === att.userId)?.className}</div></td>
                      <td className="px-6 py-4 text-center"><span className="text-[10px] font-black text-[#1CB0F6] bg-[#DDF4FF] px-2 py-1 rounded-lg uppercase">{packages.find(p => p.id === att.pkgId)?.title.substring(0, 15)}...</span></td>
                      <td className="px-6 py-4 text-center"><span className={`font-black text-lg ${att.passed ? 'text-[#58CC02]' : 'text-[#FF4B4B]'}`}>{Math.round(att.score)}</span></td>
                      <td className="px-6 py-4 text-center"><span className={`inline-block px-3 py-1 rounded-xl text-xs font-black uppercase tracking-wider ${att.passed ? 'bg-[#58CC02]/10 text-[#58CC02]' : 'bg-[#FF4B4B]/10 text-[#FF4B4B]'}`}>{att.passed ? 'LULUS' : 'REMEDIAL'}</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
